    ...
    for (const term of queriedTerms) {
        ...
        if (inlineScripts !== undefined) {
            for (const script of inlineScripts) {
                ...
                const inlineId = await assignScriptToCrawlAsTagInlineScript(...inlineScriptArgs);

                // Some inline scripts will have a best guess of what
                // remote script URL contains the mining library code.
                // If we have some guess of what that might be, we record
                // that association here.
                const suspectScript = script.suspectScript;
                if (suspectScript !== undefined) {
                    await insertRemoteScriptSuspect(
                        client, crawlId, inlineId,
                        suspectScript.url, suspectScript.numNodesPrev,
                        suspectScript.responds, logger
                    );
                }
            }
        }
...
async function insertRemoteScriptSuspect (
    client, crawlId, inlineScriptId, url,
    numNodesPrev, responds, logger
) {
    const insertQuery = `
        INSERT INTO
            remote_script_suspects(crawl_id, inline_script_terms_id, url, num_nodes_prev, responds)
        VALUES
            ($1, $2, $3, $4, $5)
        RETURNING
            id;
    `;
    const params = [crawlId, inlineScriptId, url, numNodesPrev, responds];
    logger(params);
    const newId = await getIdForQuery(client, insertQuery, params);
    logger(`Finished recording remote script with url ${url}.`);
    return newId;
}
