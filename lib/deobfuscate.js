...
var deobfuscate = function (script, debug) {
    let endScript = script;
    const captureFunc = inputString => {
        endScript = js_beautify(inputString);
    };
    ...
    const fauxWindow = {
        ...
        write: captureFunc,
        writeln: captureFunc,
        createPopup: captureFunc,
        createElement: captureFunc,
        appendChild: captureFunc,
        window: {
            eval: captureFunc,
            write: captureFunc,
            writeln: captureFunc,
    ...
    try {
        vm.runInNewContext(script, fauxWindow);
        if (debug === true) {
            console.log(`deobfuscate: Successfully deobfuscated script`);
        }
        return endScript;
    } catch (error) {
        if (debug === true) {
            console.log(`deobfuscate: Error deobfuscating script, using original.`);
        }
        return script;
    }
};
